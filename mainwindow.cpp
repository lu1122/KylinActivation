...
void MainWindow::setupTableView()
{
    ...
    model->setHeaderData(5, Qt::Horizontal, "LICENSE文件");
    model->setHeaderData(6, Qt::Horizontal, "KYINFO文件");
    
    tableView->setModel(model);
    tableView->setColumnHidden(0, true); // 隐藏ID列
    tableView->setColumnHidden(7, true); // 隐藏创建时间列
...
{
    QSqlQuery query;
    query.prepare("INSERT INTO activation_codes (serial_code, activation_code, project_number, "
                 "chassis_serial, license_file, kyinfo_file) "
                 "VALUES (:serial, :code, :project, :chassis, :license, :kyinfo)");
    
    query.bindValue(":serial", serial);
    query.bindValue(":code", code);
    query.bindValue(":project", project);
    query.bindValue(":chassis", chassis);
    query.bindValue(":license", licenseFile);
    query.bindValue(":kyinfo", kyinfoFile);
...
void MainWindow::onExportButtonClicked()
{
    ...
    // 写入表头
    out << tr("序列码,激活码,项目号,机箱序列号,LICENSE文件,KYINFO文件\n");
    
    // 写入数据
    QSqlQuery query("SELECT serial_code, activation_code, project_number, "
                   "chassis_serial, license_file, kyinfo_file FROM activation_codes");
    while (query.next()) {
